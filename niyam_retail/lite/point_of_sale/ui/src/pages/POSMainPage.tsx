...
// Icon mapping for dynamic categories from API
const ICON_MAP: Record<string, any> = {
  Grid: Grid3X3, Package: Package, Cpu: Zap, Shirt: Shirt,
  Sparkles: Sparkles, Users: Users, Baby: Baby, Watch: Watch,
  Tag: Tag, ShoppingBag: ShoppingBag, Gift: Gift,
};

const getIconComponent = (iconName: string | any) => {
  if (typeof iconName === 'string') return ICON_MAP[iconName] || Package;
...
export default function POSMainPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'cash' | 'card' | 'upi' | 'wallet' | 'credit' | 'gift_card'>('cash');
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        console.log('[POS] Using default categories');
      }
    };
    loadCategories();
  }, []);
          ...
          <Select value={paymentMethod} onValueChange={(v) => setPaymentMethod(v as typeof paymentMethod)}>
            <SelectTrigger className="w-full">
              <div className="flex items-center gap-2">
                ...
                {paymentMethod === 'gift_card' && <Gift className="h-4 w-4" />}
                <span>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              {paymentMethod === 'gift_card' && <Gift className="h-5 w-5 text-blue-600" />}
              <span className="text-sm font-medium capitalize">
                {paymentMethod === 'gift_card' ? 'Gift Card' : paymentMethod} Payment
              </span>
            </div>
          </div>
          <DialogButtons
