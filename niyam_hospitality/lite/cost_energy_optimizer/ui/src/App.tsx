...
function App() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["cost_energy_optimizer"],
    queryFn: () => api.get("/api/cost_energy_optimizer"),
  });
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b px-6 py-4">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <h1 className="text-xl font-bold">cost energy optimizer</h1>
          <ThemeToggle />
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl space-y-6 p-6">
        <PageHeader
          title="cost energy optimizer"
          description="Manage your cost energy optimizer"
        />
