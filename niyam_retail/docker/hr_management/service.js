...
const EmpCreate=z.object({ name: z.string().min(1), email: z.string().email().optional(), role: z.string().optional() });

app.get('/employees', requireAnyRole(['manager','admin','ops']), async (req,res,next)=>{
  try{ const tenantId=getTenantId(req); const r=await query('SELECT * FROM employees WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT 200',[tenantId]); res.json({ success:true, employees:r.rows }); }catch(e){ next(e); }
});
app.post('/employees', requireAnyRole(['manager','admin','ops']), async (req,res,next)=>{
  try{ const tenantId=getTenantId(req); const b=EmpCreate.parse(req.body); const r=await query('INSERT INTO employees (tenant_id, name, email, role, status) VALUES ($1,$2,$3,$4,\'active\') RETURNING *',[tenantId,b.name,b.email||null,b.role||null]); res.json({ success:true, employee:r.rows[0] }); }catch(e){ next(e); }
});

app.get('/healthz', (req,res)=> res.json({ status:'ok', service:'hr_management' }));
app.get('/readyz', (req,res)=> res.json({ status:'ready', service:'hr_management' }));
...
app.listen(PORT, ()=> console.log('HR Management listening on', PORT));
