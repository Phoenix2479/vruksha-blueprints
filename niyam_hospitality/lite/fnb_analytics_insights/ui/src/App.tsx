...
function App() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["fnb_analytics_insights"],
    queryFn: () => api.get("/api/fnb_analytics_insights"),
  });
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b px-6 py-4">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <h1 className="text-xl font-bold">fnb analytics insights</h1>
          <ThemeToggle />
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl space-y-6 p-6">
        <PageHeader
          title="fnb analytics insights"
          description="Manage your fnb analytics insights"
        />
