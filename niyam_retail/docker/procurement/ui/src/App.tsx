...
import { FileText, RefreshCw, CheckCircle, XCircle, Plus, Loader2 } from 'lucide-react'
...
export default function App() {
  ...
  const tabs = ["Dashboard","Purchase Orders","Actions"]
            ...
            {tabs.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`py-4 px-1 border-b-2 font-medium text-sm ${
                  activeTab === tab ? 'border-cyan-500 text-cyan-600' :
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatsCard label="Service" value="Procurement" />
      </div>
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <EntityForm
            fields={[{"name":"supplier_id","label":"Supplier ID","type":"text","required":true},{"name":"items","label":"Items","type":"json","required":true},{"name":"expected_date","label":"Expected Date","type":"date"},{"name":"notes","label":"Notes","type":"textarea"}]}
      ...
      <DataTable
        columns={["id","supplier_id","status","expected_date","created_at"]}
        data={items}
        isLoading={isLoading}
      />
    </div>
...
function ActionsView() {
  ...
  const actions = [{"name":"Receive Order","endpoint":"/purchase-orders/:id/receive","method":"POST"},{"name":"Rate Supplier","endpoint":"/suppliers/:id/rate","method":"POST"}]
